		...
		if ($primary && isset($results[0][$model->alias][$classField]) && $model->recursive > 0) {
			foreach ($results as $key => $result) {
				...
				if ($class && $foreignId) {
					...
					$fields = isset($model->$class->hasMany[$model->alias]['fields']) && !empty($model->$class->hasMany[$model->alias]['fields']) ? $model->$class->hasMany[$model->alias]['fields'] : array('id', $model->$class->displayField);
					$associated = $model->$class->find(array($class . '.id' => $foreignId), $fields, null, -1);
