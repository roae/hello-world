...
class ArosController extends AclAppController {
	...
	function admin_role_permissions()
	{
		...
		$this->set("controllers",$this->__getControllers());

	}

	function __getControllers(){
		$controllers['app']= Set::extract("/name",$this->AclReflector->get_all_app_controllers());
		$plugins=Set::extract("/name",$this->AclReflector->get_all_plugins_controllers());
		foreach($plugins as $plugin){
			list($_plugin, $controller )=explode("/",$plugin);
			$controllers['plugins'][$_plugin][]=$controller;

		}
		return $controllers;
	}

	function admin_controller_role_permissions($controller = null,$plugin = null){
		$actions=$this->AclReflector->get_controller_actions($controller);
		$permissions = array();
		$methods     = array();
		...
		foreach($actions as $action){
			$full_action=$plugin ? $plugin."/": "";
			$full_action.=implode("/",array($controller,$action));
			...
			if($plugin){
				$methods[] = array('name' => $action, 'permissions' => $permissions);
			}else{
				$methods[] = array('name' => $action, 'permissions' => $permissions);
			}
		}
		...
		//debug($methods);
		$this->set("controller_name",$controller);
		$this->set("plugin_name",$plugin);
		$this->set('roles', $roles);
	...
	function admin_controller_user_permissions($user_id = null, $controller = null,$plugin = null){
		$user_model_name = Configure :: read('acl.aro.user.model');
		$this->{$user_model_name}->recursive = -1;
		$user = $this->{$user_model_name}->read(null, $user_id);
		$this->set('user', $user);
		...
		foreach($actions as $action){
			...
			if(!empty($aco_node)){
				...
				$permissions = $authorized ? 1 : 0 ;
			}
			
			/*if(isset($plugin)){
				$methods['plugin'][$plugin][$controller][] = array('name' => $action, 'permissions' => $permissions);
			}else{
				$methods[] = array('name' => $action, 'permissions' => $permissions);
			}*/
			$methods[] = array('name' => $action, 'authorized' => $authorized);
		}
		//pr($methods);
		$this->set("controller_name",$controller);
		$this->set("plugin_name",$plugin);
