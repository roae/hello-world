...
jQuery.fn.createAppend = function(element, attrs, content)
{
    ...
    // I *hate* exceptions
    if (isIE && element == 'input' && attrs.type) {
    ...
    if (/*isIE && */parentElement.nodeName.toLowerCase() == 'table' && element.nodeName.toLowerCase() == 'tr') {
...
jQuery.create = function(element, attrs, content){
	var e;
	if (attrs == undefined && element.constructor == Array) {
		for (var i = 0; i < element.length; i += 3) {
			if(!i){
				e=$.create(element[i], element[i + 1] || {}, element[i + 2] || []);
			}else{
				e.after($.create(element[i], element[i + 1] || {}, element[i + 2] || []));
			}
		}
		return e;
	}
	 ...
	 if (isIE && element == 'input' && attrs.type) {
        ...
        // IE will not allow you to modify the type attribute after an element is
        // created, so we must create the input element with the type attribute.
        var e = document.createElement('<' + element + ' type="' + attrs.type + '" />');
    ...
    if (typeof content == 'object' && content != null) {
        ...
        for (var i = 0; i < content.length; i = i + 3) {
            ...
            jQuery(e).createAppend(content[i], content[i + 1] || {}, content[i + 2] || []);
...
jQuery.fn.tplPrepend = function(json, tpl) {
    ...
    for (i = div.childNodes.length - 1; i >= 0; i--) {
        ...
        if (isIE && self.nodeName.toLowerCase() == 'table' && div.childNodes[i].nodeName.toLowerCase() == 'tr') {
