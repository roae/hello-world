<?php /* @var $this View */
$this->Html->addCrumb('[:System.admin_cities_list:]');
...
if(!empty($recordset)){
	...
	echo $this->Html->tag("div",
		$this->Html->tag("div",
			...
			,"lTools").
		$this->Html->tag("div",
			#$this->Html->tag('button',$this->Html->tag("i","","icon-circle")."<span>[:publish:]</span>",array('type'=>'submit','title'=>'data[Xpagin][url]','value'=>$this->Html->url(array('action'=>'status',1)),'onclick'=>'return paginAction(this,"[:message_confirm_publish_mutiple_cities:]")','class'=>'btn btn_primary')).
			#$this->Html->tag('button',$this->Html->tag("i","","icon-circle-blank")."<span>[:unpublish:]</span>",array('type'=>'submit','title'=>'data[Xpagin][url]','value'=>$this->Html->url(array('action'=>'status',0)),'onclick'=>'return paginAction(this,"[:message_confirm_unpublish_mutiple_cities:]")','class'=>'btn')).
			$this->Html->tag('button',$this->Html->tag("i","","icon-remove-sign")."<span>[:delete_city:]</span>",array('type'=>'submit','name'=>'data[Xpagin][url]','value'=>$this->Html->url(array('action'=>'delete')),'onclick'=>"return paginAction(this,'[:warning_message_delete_multiple_cities:]')",'class'=>'btn btn_danger')).
			$this->Html->link($this->Html->tag("i","","icon-plus")."<span>[:add_city:]</span>",array('action'=>'add'),array('escape'=>false,'class'=>'btn btn_success'))
			,array('class'=>'rTools'))
	...
	foreach((array)$recordset as $count=>$record){
		$actions=$this->Html->div('btn-group',
			$this->Paginator->link($this->Html->tag("i","","icon-pencil")."[:editar:]",array('action'=>'edit',$record['City']['id']),array('rev'=>'','class'=>'btn btn-info','escape'=>false)).
			$this->Form->button("<span class='caret'></span>",array('type'=>'button','class'=>'btn btn-info  dropdown-toggle','data-toggle'=>'dropdown')).
			$this->Html->tag("ul",
				$this->Html->tag("li",$this->Paginator->link("[:list_locations:]",array('controller'=>'locations','action'=>'index','city'=>$record['City']['id']))).
				$this->Html->tag("li",$this->Paginator->link("[:add_location:]",array('controller'=>'locations','action'=>'add','city'=>$record['City']['id']))).
				$this->Html->tag("li","","divider").
				$this->Html->tag("li",$this->Paginator->link("<span class='icon-trash'></span> [:delete:]",array('action'=>'delete',$record['City']['id']),array('class'=>'action danger','rel'=>'[:System.delete_city_name:]: '.h($record['City']['name']).'?','escape'=>false)))
	...
	echo $this->Html->div("noRecords",
		...
		$this->Html->tag("div","[:System.cities_not_found:] ".$this->Html->tag("strong",h($this->data['Xpagin']['search']))).
	...
	echo $this->Html->div("noRecords",
		...
		$this->Html->tag("div","[:System.no_cities_yet:]").
		$this->Html->link($this->Html->tag("i","","icon-plus")."[:add_city:]",array('action'=>'add'),array('class'=>'btn btn_success','escape'=>false))
	);
}
