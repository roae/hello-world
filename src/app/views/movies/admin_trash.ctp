<?php /* @var $this View */
$this->Html->addCrumb('[:System.admin_movies_trash:]');
...
if(!empty($recordset)){
	...
	echo $this->Html->tag("div",
		$this->Html->tag("div",
			...
			,"lTools").
		$this->Html->tag("div",
			$this->Html->tag('button',$this->Html->tag("i","","icon-remove-sign")."<span>[:delete_all:]</span>",array('type'=>'submit','name'=>'data[Xpagin][url]','value'=>$this->Html->url(array('action'=>'destroy')),'onclick'=>"return paginAction(this,'[:warning_message_delete_multiple_movies:]')",'class'=>'btn btn_danger'))
			,array('class'=>'rTools'))
	...
	foreach((array)$recordset as $count=>$record){
		$actions=$this->Html->div('btn-group',
			$this->Paginator->link($this->Html->tag("i","","icon-ok-sign")." [:restore:]",array('action'=>'restore',$record['Movie']['id']),array('class'=>'action btn btn_primary noHistory','escape'=>false)).
			$this->Paginator->link($this->Html->tag("i","","icon-remove-sign"),array('action'=>'destroy',$record['Movie']['id']),array('class'=>'action btn btn_danger','rel'=>'[:delete_city_name:]: '.h($record['Movie']['title']).'?','escape'=>false,'title'=>'[:destroy:]')
	...
	echo $this->Html->div("noRecords",
		...
		$this->Html->tag("div","[:System.movies_not_found_trash:] ".$this->Html->tag("strong",h($this->data['Xpagin']['search']))).
	...
	echo $this->Html->div("noRecords",
		...
		$this->Html->tag("div","[:System.no_movies_yet_in_trash:]").
		$this->Html->link($this->Html->tag("i","","icon-plus")."[:go_movies_list:]",array('action'=>'index'),array('class'=>'btn btn_success','escape'=>false))
	);
}
